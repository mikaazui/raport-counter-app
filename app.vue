      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-4">
          <div v-for="i in 7" :key="i">
            <span>Pekan {{ i }}</span>
            <input class="input input-bordered" v-model="data[`nilai_p${i}`]" type="number" />
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
          <div v-for="(label, key) in kelasInputs" :key="key" class="grid">
            <span>{{ label }}</span>
            <input class="input input-bordered " v-model="data[key]" type="number" />
          </div>
        </div>
      </div>

      <!-- Result Section -->
       <div class="bg-pink-600 rounded-full h-[670px] grid place-items-center">
         <div class="w-full h-full flex flex-col justify-center items-center pb-[60px]">
           ...
           <p class="text5xl font-bold absolute bottom-[280px]">Nilai Akhir</p>
         </div>
       </div>
      </div>
    </div>
...
const data = ref({
  nilai_p1: 0, nilai_p2: 0, nilai_p3: 0, nilai_p4: 0, nilai_p5: 0, nilai_p6: 0, nilai_p7: 0,
  nilai_adab: 0, nilai_akhlak: 0, nilai_us: 0
})

// Input labels for class-related values
const kelasInputs = {
  nilai_adab: 'Nilai Adab',
  nilai_akhlak: 'Nilai Akhlak',
  nilai_us: 'Nilai US'
}
...
const sum_nilai_up = () => {
  ...
  const total_adab_akhlak = data.value.nilai_adab + data.value.nilai_akhlak
